# -*- coding: utf-8 -*-
#
# hpc_benchmark_strong_scaling.py
#
...
params = {
    ...
    'use_recurrent_input': True,   # Use a predefined spike pattern
...
brunel_params = {
    ...
    'NE_proc': int(9000 * params['scale'])//mpi_np,
    'NI_proc': int(2250 * params['scale'])//mpi_np,
...
def build_network():
    ...
    if(mpi_np > 1):
        dum_neurons = brunel_params['NE_proc'] + brunel_params['NI_proc']
        dum_NE = brunel_params['NE_proc']
        dum_NI = brunel_params['NI_proc']
        for i in range(mpi_np):
            neurons.append(ngpu.RemoteCreate(i, 'iaf_psc_alpha', dum_neurons, 1, model_params).node_seq)
            E_pops.append(neurons[i][0:dum_NE])
            I_pops.append(neurons[i][dum_NE:dum_neurons])
    ...
    if brunel_params['randomize_Vm']:
        ...
        print(ngpu.GetStatus(neurons[mpi_id], "V_m_rel"))
        membrane_potentials = ngpu.GetStatus(neurons[mpi_id], "V_m_rel")
        np.savetxt("vm_rank_{}_np_{}.dat".format(mpi_id,mpi_np), membrane_potentials)
    ...
    rate = nu_ext * CE * 1000.
    print("Rate expected [Hz]: ", rate)
    if not params['use_dc_input'] and not params['use_recurrent_input']:
        brunel_params["poisson_rate"] = rate
        E_stim= ngpu.Create('poisson_generator', 1, 1, {'rate': rate})
    elif params['use_recurrent_input']:
        np.random.seed(1234)
        spiketimes = np.round(np.sort(np.random.random(math.ceil(rate*(params['presimtime']+params['simtime'])/1000.))*(params['presimtime']+params['simtime'])), 1)
        unique, occurrencies = np.unique(spiketimes, return_counts=True)
        E_stim = ngpu.Create("spike_generator")
        ngpu.SetStatus(E_stim, {"spike_times": unique, "spike_heights": occurrencies})
    else:
        ...
        #print("I_amp [pA]: {} E_amp [pA]: {}".format(inh_amp, ex_amp))
    ...
    if params['record_spikes']:
        ...
        record = ngpu.CreateRecord("", ["V_m_rel", "V_m_rel", "V_m_rel"], [neurons[mpi_id][0], neurons[mpi_id][1], neurons[mpi_id][2]], [0, 0, 0])
    ...
    if not params["use_dc_input"]:
        ...
        # connect Poisson generator or spike_generator to neuron
    ...
    for i in range(mpi_np):
        for j in range(mpi_np):
            if(i!=j):
                rank_print('Connecting excitatory {} -> population {}.'.format(i, j))

                my_remoteconnect(i, E_pops[i], j, neurons[j],
                                    e_conn_rule, syn_dict_ex)

                rank_print('Connecting inhibitory {} -> population {}.'.format(i, j))
...
def run_simulation():
    ...
    if params['record_spikes']:
        ...
        if params['show_plot']:
            ...
            V_m1 = [row[1] for row in recorded_data]
            V_m2 = [row[2] for row in recorded_data]
            V_m3 = [row[3] for row in recorded_data]
            plt.figure(mpi_id)
            plt.plot(time, V_m1, '-r')
            plt.plot(time, V_m2, '-b')
            plt.plot(time, V_m3, '-g')
            plt.draw()
...
def get_spike_times(neurons):
    ...
    e_count = 0
    e_data = []
    e_bound = brunel_params['NE_proc']
    i_count = 0
    i_data = []
    i_bound = (brunel_params['NE_proc'] + brunel_params['NI_proc'])
    ...
    return (brunel_params['NE_proc'], e_count), e_data, (brunel_params['NI_proc'], i_count), i_data
